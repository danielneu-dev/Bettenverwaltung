...
CREATE TABLE d_geschlecht(
						 geschlecht VARCHAR(1) PRIMARY KEY
                         );
CREATE TABLE d_status(
					 status VARCHAR(12) PRIMARY KEY
                     );
...
CREATE TABLE bett(
				 bettID INT PRIMARY KEY AUTO_INCREMENT,
...
VALUES ('belegt'),('frei'),('frei'),('kontaminiert'),
	   ('kontaminiert'),('belegt'),('belegt'),('belegt'),('belegt');
INSERT INTO bett (bettID, status) VALUES (9558,'frei');
...
SELECT
    ...
    (
        SELECT COUNT(*)
        FROM reinigung r
        WHERE r.bettID = p1.bettID
        AND r.datum > p1.aufnahmedatum
